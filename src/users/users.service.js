...
let UsersService = class UsersService {
    ...
    async createUser(createUserDto) {
        ...
        try {
            ...
            if (error.code === 'ER_DUP_ENTRY') {
                throw new common_1.HttpException(`'${email}' is already exist.`, common_1.HttpStatus.CONFLICT);
            }
            else
    ...
    async editUser(editUser, userStat) {
        ...
        if (+id === userStat.id || userStat.role >= user_entity_1.RolePermitted.mentor) {
            user.firstName = firstName;
            user.lastName = lastName;
            user.userName = userName;
            user.gender = gender;
        }
        if (+id === userStat.id || userStat.role >= user_entity_1.RolePermitted.moderator) {
            user.mobile = mobile;
